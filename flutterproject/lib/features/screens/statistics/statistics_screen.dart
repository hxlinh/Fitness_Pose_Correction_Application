...
class ExerciseStatsScreen extends StatefulWidget {
  ...
  const ExerciseStatsScreen({super.key, required this.currentExercise});
...
class _ExerciseStatsScreenState extends State<ExerciseStatsScreen> {
  ...
  final Trace trace = FirebasePerformance.instance.newTrace('statistics_end');
  trace.start();
  ...
  historyRef.onValue.listen((event) {
    if (event.snapshot.value != null) {
      setState(() {
        ...
        totalReps = data['reps'] ?? 0; // Lấy tổng số lần thực hiện
      });
    }
  });
            ...
            Expanded(
              child: DataTable(
                columns: const [
                  ...
                  DataColumn(label: Text("% of Reps")),
                ],
                rows: errorCount.entries.map((entry) {
                  final errorDescription = getErrorDescription(entry.key);
                  final count = entry.value;
                  final percentage = totalReps > 0 ? (count / totalReps * 100).toStringAsFixed(2) : "0";
              ...
              child: PieChart(
                PieChartData(
                  ...
                  sections: errorCount.entries.map((entry) {
                    final percentage = totalReps > 0 ? (entry.value / totalReps * 100) : 0;
