...
class SetupScreenSquat extends StatefulWidget {
  const SetupScreenSquat({super.key});

  @override
  _SetupScreenSquatState createState() => _SetupScreenSquatState();
}

class _SetupScreenSquatState extends State<SetupScreenSquat> {
  String Exercise = 'Squat'; // Động tác cố định
  int? selectedReps; // Biến lưu số rep đã chọn
  List<String> exerciseList = []; // Danh sách bài tập đã thêm

  // Hàm gửi dữ liệu lên Firebase
  void sendDataToFirebase() async {
    final trace = FirebasePerformance.instance.newTrace('setting_squat');
    await trace.start(); // Bắt đầu theo dõi
    final databaseRef = FirebaseDatabase.instance.ref('history_squat');

    // Đọc số thứ tự hiện tại
    final entryCountSquatSnapshot = await databaseRef.child('entryCountSquat').get();
    int entryCountSquat = entryCountSquatSnapshot.exists ? entryCountSquatSnapshot.value as int : 0;
    ...
    await databaseRef.child(entryCountSquat.toString()).set({
    ...
    await databaseRef.child('entryCountSquat').set(entryCountSquat);
            ...
            Padding(
              ...
              child: TextField(
                decoration: InputDecoration(
                  ...
                  labelText: 'Nhập số rep', // Nhãn cho ô nhập liệu
                  hintText: 'Nhập số rep bạn muốn thực hiện', // Hướng dẫn cho người dùng
                ...
                onChanged: (value) {
                  setState(() {
                    selectedReps = int.tryParse(value); // Cập nhật số rep đã chọn
                  });
                },
              ),
            ),
